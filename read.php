<?PHP
	...
	if(isset($_POST['select']) && $_POST['select'] == "obv"){
		$dato1 = $_POST['dato1'];
		$query = "SELECT * FROM Observacion_voto WHERE Reporte_idReporte = '1'";
	}
	...
	if(isset($_POST['select']) && $_POST['select'] == "prof2"){
		...
		$query = "SELECT * FROM Profesor p WHERE idprofesor IN (
			SELECT Profesor_idProfesor from clase where idClase = '$dato1'
		);";
	}
